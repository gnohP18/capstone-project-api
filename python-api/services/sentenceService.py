...
class ModelSentence(metaclass=SingletonMeta):
    ...
    def loadModel(self):
        ...
        if self.tokenizer is None:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(constant.PHOBERT_BASE_PATH, use_fast=True)
    ...
    def tokenizeArrayObject(self, arr):
        return [self.tokenizer(item, return_tensors="pt", truncation=True, padding=True) for item in arr]
    ...
    def padAndConcat(self, tensors):
        ...
        for tensor in tensors:
            ...
            if padLength > 0:
                paddedTensor = torch.cat([tensor, torch.zeros(tensor.size(0), padLength, dtype=tensor.dtype)], dim=1)
            else:
                paddedTensor = tensor
